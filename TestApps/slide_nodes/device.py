...
def _prepare_document(path, log):
    ...
    from rpstack.primitive_runtime.manifest import load_document, load_entry_point, ServiceManifest
...
class _Session:
    def __init__(self, role, provider_path, client_path, verbose):
    ...
    async def run(self):
        ...
        try:
            ...
            while not self.read()['stop']:
                ...
                if provider is not None and provider.state == 'failed':
...
def start(role="slide", provider_path=None, client_path=None, stack_size=65536, verbose=True):
...
def demo(position_mm=100, timeout_s=30):
    """Run node2 once, print its signals, and return its final PositionSample."""
    session = _session
    if session is None:
...
def status():
    ...
    with _session.lock:
        ...
        return {'state': state['state'], 'node1': state['node1'],
                'demo': demo_state, 'error': state['error'] or (job['error'] if job else None)}
