...
from rpstack.signals.model import asyncio, json, now_ms, elapsed, encode
...
class Catalog:
    ...
    def _prune(self):
        for key, record in tuple(self.nodes.items()):
            if elapsed(record['seen']) >= self.ttl_ms:
                del self.nodes[key]
    ...
    def snapshot(self):
        ...
        for key in sorted(self.nodes):
            ...
            profile['age_ms'] = max(0, elapsed(record['seen']))
    ...
    async def run(self):
        ...
        while True:
            ...
            if not frames and (last is None or elapsed(last) >= self.interval_ms):
                frames = self.frames()
                last = now_ms()
            if frames:
                try:
