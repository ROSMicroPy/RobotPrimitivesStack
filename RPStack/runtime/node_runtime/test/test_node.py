...
from rpstack.support import asyncio as runtime_asyncio

from rpstack.shell.async_shell import AsyncShell


class Pin:
    # Create a numbered fake output pin initially held low.
    def __init__(self, number):
        self.number, self.state = number, 0
    # Record the latest logical pin level for cleanup assertions.
    def value(self, value):
        self.state = value


class MotorDriver(StepDirDriver):
    # Inject fake pins while retaining the real step/dir initialization path.
...
class DistanceDriver:
    # Simulate successful asynchronous sensor initialization.
    async def initialize(self, **kwargs):
        self.active = True
        return True
    # Yield briefly before returning a fixed distance to exercise concurrent controls.
    async def read_distance_mm(self):
        await asyncio.sleep(0.002)
        return 100
    # Record sensor activation changes requested by the node.
    def set_active(self, active):
        self.active = active
        return True
    # Mark the fake distance driver inactive during cleanup.
    def shutdown(self):
        self.active = False
        return True
    # Expose the fake sensor's active flag for status checks.
    def get_status(self):
        return {'active': self.active}


class FailingDriver(DistanceDriver):
    # Inject a startup failure to exercise partial node cleanup.
    async def initialize(self, **kwargs):
        raise RuntimeError('sensor initialization failed')


# Load the slide deployment and replace hardware/app settings with local integration-test
# fixtures.
def document():
    doc = json.loads((ROOT / 'examples/linear_slide/manifest.json').read_text())
...
class NodeTests(unittest.IsolatedAsyncioTestCase):
    # Boot a simulated slide node and locate its ephemeral HTTP port.
    ...
    # Shut down the test node and all resources it owns.
    async def asyncTearDown(self):
        await self.node.shutdown()

    # Send one JSON HTTP request to the node and decode its status/body.
    ...
    # Verify dependency startup, resident task visibility, shell listing, and public discovery
    # output.
    ...
    # Exercise concurrent motion, status, conflicts, stop, and reset through HTTP.
    ...
    # Verify signals resume workflows and node stop cancels waiting flows and subscriptions.
    async def test_event_flow_signal_and_stop_waiting_flow(self):
        flow_id = self.node.start_flow('observe_on_signal')
        await asyncio.sleep(0)
        self.node.engine.signals.publish('observe', {'source': 'test'})
        ...
        self.assertEqual(self.node.engine.signals.subscribers, {})

    # Reject malformed operation inputs while serving independent discovery requests
    # concurrently.
    ...
    # Ensure a partial request cannot block another client's stop command.
    ...
    # Verify reset stops old motion before admitting work on reconstructed services.
    ...
    # Ensure sensor startup failure releases already-initialized motor resources and tasks.
    ...
    # Verify cancellation before an operation runs still releases its reservations.
    ...
    # Ensure resident service failure closes admission and health monitoring survives a reset.
    async def test_service_failure_stops_motion_and_health_survives_reset(self):
        # Inject an immediate resident polling failure for health-supervision assertions.
        async def fail():
    ...
    # Check repeated service reconstruction leaves one resident task per service and no stale
    # claims.
    ...
    # Saturate workflow capacity and verify reserved capacity still permits HTTP shutdown
    # controls.
    ...
    # Ensure incompatible dependencies fail validation before hardware factories are called.
    ...
    # Verify a reset queued behind lifecycle control cannot restart a shutting-down node.
    ...
    # Exercise calibration submission, task completion, status transitions, and invalid HTTP
    # arguments.
    async def test_explicit_calibration_endpoint_and_status(self):
        code, status = await self.request('GET', '/api/node/status')
        self.assertEqual(status['calibration']['slide']['state'], 'pending')
        self.assertEqual(status['calibration']['motor']['state'], 'not_required')
        # Return deterministic calibration results without moving hardware.
        async def calibrate(steps=1000):
            return {'steps': steps}
        self.node.instance('slide').calibrate = calibrate
        code, job = await self.request('POST', '/api/node/calibrate',
                                       {'service': 'slide', 'arguments': {'steps': 200}})
        self.assertEqual(code, 202)
        self.assertEqual(await self.node.tasks.wait(job['task_id']), {'steps': 200})
        code, status = await self.request('GET', '/api/node/status')
        self.assertEqual(status['calibration']['slide']['state'], 'calibrated')
        self.assertEqual((await self.request('POST', '/api/node/calibrate', {'automatic': True}))[0], 422)

    # Exercise signal-triggered workflow completion and reject reserved or invalid signal
    # requests.
