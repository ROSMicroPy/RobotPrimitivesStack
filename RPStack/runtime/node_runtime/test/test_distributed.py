...
class DistributedTests(unittest.IsolatedAsyncioTestCase):
    # Boot three simulated robot nodes with short leases and manually controlled workflows.
    async def asyncSetUp(self):
        self.nodes = {}
        for name in ('arm', 'base', 'controller'):
            doc = json.loads((ROOT / 'examples/robie1' / (name + '.json')).read_text())
    ...
    # Shut down nodes in reverse order and assert transport/task cleanup.
    ...
    # Wait for a distributed state change within a bounded test deadline.
    async def until(self, predicate, timeout=1):
        # Yield between predicate checks so message delivery and worker tasks can progress.
        async def poll():
            while not predicate():
                await asyncio.sleep(0.005)
        await asyncio.wait_for(poll(), timeout)

    # Build a one-step workflow invoking a remote arm movement.
    ...
    # Verify a multi-node manifest workflow executes once per worker and publishes terminal
    # state.
    ...
    # Exercise a remote call across the mesh-to-ROS bridge.
    ...
    # Replay a completed call identity and verify the remote action is not executed twice.
    ...
    # Ensure stopping the coordinator cancels the worker's active motion.
    ...
    # Drop cancellation and renewals to verify the receiver eventually stops motion on lease
    # expiry.
    ...
    # Verify periodic renewals sustain an action longer than its initial lease.
    ...
    # Check that reset rejects old-epoch calls while fresh handshakes still work.
    ...
    # Deliver cancellation before its call and verify the delayed call cannot start motion.
    ...
    # Reject remote work for unexposed services and undeclared peers.
    ...
    # Ensure an older state revision cannot overwrite an observed terminal outcome.
    ...
    # Verify simultaneous workflow waits consume only their own correlated signals.
    ...
    # Check that a manifest-configured distributed workflow starts during node boot.
    async def test_manifest_autostarts_distributed_flow(self):
        await self.controller.shutdown()
        self.controller = NodeRuntime.load(str(ROOT / 'examples/robie1/controller.json'))
    ...
    # Verify service signals can satisfy workflow waits established before remote invocation.
    ...
    # Ensure failed transport supervision closes admission and requires a full node restart.
    ...
    # Verify repeated shutdown does not close an owned transport more than once.
    async def test_repeated_shutdown_closes_transports_once(self):
        ...
        # Count transport shutdown calls while preserving the original cleanup behavior.
        async def stop():
            calls.append('stop')
            await original()
        transport.stop = stop
