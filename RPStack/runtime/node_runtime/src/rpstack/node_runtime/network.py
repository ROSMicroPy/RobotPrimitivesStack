...
class WifiService:
    # Store credential variable names and the connection deadline without activating hardware.
    ...
    # Activate station mode and connect using environment credentials within the configured
    # timeout.
    async def start(self):
        ...
        if not self.wlan.isconnected():
            ...
            # Yield until the WLAN reports a connection; the caller supplies the deadline.
            async def connected():
                while not self.wlan.isconnected():
                    await asyncio.sleep(0.1)
            await asyncio.wait_for(connected(), self.timeout_s)

    # Monitor the connection and surface a lost link as a resident service failure.
    async def run(self):
        while True:
            if not self.wlan.isconnected():
    ...
    # Disconnect the WLAN if startup created a network interface.
    def stop(self):
        if self.wlan:
