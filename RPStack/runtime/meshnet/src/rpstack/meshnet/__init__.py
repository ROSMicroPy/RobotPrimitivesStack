"""Bounded ESP-NOW signal transport, extracted from LighthouseMesh's dnet."""
from rpstack.signals.model import asyncio, now_ms, elapsed
...
class Framing:
    ...
    def expire(self):
        for key, value in tuple(self.pending.items()):
            if elapsed(value['started']) >= self.timeout_ms:
