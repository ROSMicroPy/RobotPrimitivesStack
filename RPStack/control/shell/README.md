...
The core package excludes these commands and their bundled server helpers. Install the optional set with one command once this revision is
...
precedence over optional and registered external commands.
...
Unknown commands report an error. `python`
...
```sh
python3 RPStack/control/shell/tools/files2pkgjson.py
python3 -m unittest discover -s RPStack/control/shell/test -v
```
