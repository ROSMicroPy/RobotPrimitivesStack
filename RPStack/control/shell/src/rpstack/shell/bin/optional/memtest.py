...
# Allocate configurable test arrays and repeatedly exercise memory while displaying device
# statistics.
def run(context, arguments):
    ...
    cnt = 512 if len(args) < 1 else int(args[0])
    size = 1024 if len(args) < 2 else int(args[1])
    epochs = 10 if len(args) < 3 else int(args[2])
