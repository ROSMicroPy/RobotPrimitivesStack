...
class TelnetWrapper(uio.IOBase):
    # Wrap a socket as a terminal stream and prepare telnet-control filtering state.
    def __init__(self, socket):
        super().__init__()
        self.socket = socket
        self.discard_count = 0
        
    # Read terminal bytes while discarding telnet controls/nulls and handling nonblocking reads.
    def readinto(self, b):
        readbytes = 0
    ...
    # Keep writing until all bytes are sent, retrying temporary socket backpressure.
    def write(self, data):
    ...
    # Close the socket underlying this terminal stream.
    def close(self):
        self.socket.close()
...
# Replace the previous client, negotiate terminal mode, and attach the new socket to dupterm.
...
# Detach the remote terminal and close listener/client sockets.
def stop():
...
# Restart the telnet listener and advertise active interface addresses.
def start(port=23):
    stop()
