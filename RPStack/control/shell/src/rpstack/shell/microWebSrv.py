...
class MicroWebSrvRoute :
    # Store a route's HTTP method, handler, capture names, and compiled matching expression.
...
class MicroWebSrv :
    ...
    def route(cls, url, method='GET'):
        ...
        # Register a decorated handler and return the original callable unchanged.
    ...
    # Replace HTML-sensitive characters with their configured escape sequences.
    @staticmethod
    def HTMLEscape(s) :
        return ''.join(MicroWebSrv._html_escape_chars.get(c, c) for c in s)


    # Retry buffer allocation after garbage collection, returning None if memory remains
    # unavailable.
    @staticmethod
    ...
    # Retry worker creation using supported platform thread-start signatures.
    @staticmethod
    ...
    # Decode percent escapes while leaving malformed sequences intact.
    @staticmethod
    def _unquote(s) :
        r = s.split('%')
    ...
    # Decode form-style plus signs and percent escapes.
    @staticmethod
    def _unquote_plus(s) :
        return MicroWebSrv._unquote(s.replace('+', ' '))


    # Check whether a filesystem path can be stat-ed.
    @staticmethod
    ...
    # Recognize filenames handled by the optional Python HTML template engine.
    @staticmethod
    def _isPyHTMLFile(filename) :
        return filename.lower().endswith(MicroWebSrv._pyhtmlPagesExt)


    # Configure the listener, static-file root, WebSocket options, and parameterized route
    # patterns.
    def __init__( self,
                  routeHandlers = [],
                  port          = 80,
    ...
    # Accept and process clients until the listener closes or accept fails.
    ...
    # Create the TCP listener and run its accept loop inline or in a worker thread.
    ...
    # Close the listener to stop its accept loop when the server is running.
    def Stop(self) :
        if self._started :
            self._server.close()


    # Report the accept loop's current running flag.
    def IsStarted(self) :
        return self._started


    # Configure an optional redirect target for missing resources.
    def SetNotFoundPageUrl(self, url=None) :
        self._notFoundUrl = url


    # Choose a MIME type from the configured filename extension table.
    ...
    # Match method/path against routes and extract named arguments, converting integers when
    # possible.
    ...
    # Resolve an existing static path, checking default index pages for the root URL.
    ...
    class _client :
        ...
        # Wrap a client socket, initialize request fields, and immediately process its request.
        ...
        # Parse a request, dispatch routes/static files or protocol upgrades, and finalize the
        # connection.
        ...
        # Decode the request method, resource path, protocol, and query fields.
        ...
        # Collect header lines and record POST content metadata.
        ...
        # Return the requested upgrade protocol when Connection advertises an upgrade.
        ...
        # Return the server that accepted this client.
        def GetServer(self) :
            return self._microWebSrv


        # Return the client's remote socket address tuple.
        def GetAddr(self) :
            return self._addr


        # Return the remote client's IP address.
        def GetIPAddr(self) :
            return self._addr[0]


        # Return the remote client's TCP port.
        def GetPort(self) :
            return self._addr[1]


        # Expose the parsed HTTP method to route handlers.
        def GetRequestMethod(self) :
            return self._method


        # Return the original request target including its query string.
        def GetRequestTotalPath(self) :
            return self._path


        # Return the decoded resource path without the query string.
        def GetRequestPath(self) :
            return self._resPath


        # Return the raw query-string portion of the target.
        def GetRequestQueryString(self) :
            return self._queryString


        # Expose decoded query parameters to route handlers.
        def GetRequestQueryParams(self) :
            return self._queryParams


        # Expose the parsed HTTP header mapping.
        def GetRequestHeaders(self) :
            return self._headers


        # Return the content type recorded for the request body.
        def GetRequestContentType(self) :
            return self._contentType


        # Return the parsed request body length.
        def GetRequestContentLength(self) :
            return self._contentLength


        # Attempt a nonblocking body read and restore blocking mode afterward.
        ...
        # Read and decode ampersand-separated form name/value pairs.
        ...
        # Decode the request body as JSON, returning None on failure.
    ...
    class _response :
        ...
        # Bind response writing to the current client connection.
        def __init__(self, client) :
            self._client = client


        # Encode text when needed and write it through the client stream.
        ...
        # Write the HTTP status line using the configured reason phrase.
        def _writeFirstLine(self, code) :
            reason = self._responseCodes.get(code, ('Unknown reason', ))[0]
            self._write("HTTP/1.1 %s %s\r\n" % (code, reason))


        # Write one HTTP header line.
        def _writeHeader(self, name, value) :
            self._write("%s: %s\r\n" % (name, value))


        # Write content type and optional charset, defaulting to binary data.
        ...
        # Identify the server implementation in the response headers.
        ...
        # Write status and response headers before sending a body.
        ...
        # Send and flush the HTTP 101 handshake for a protocol upgrade.
        ...
        # Write headers and optional content, returning whether the write succeeded.
        ...
        # Render a Python HTML template or return an error when rendering is unavailable or
        # fails.
        ...
        # Stream an existing nonempty file through a small buffer, reporting allocation/read
        # failures.
        ...
        # Add an attachment filename before streaming the requested file.
        ...
        # Send a successful response with the caller's headers and content.
        ...
        # Send a 302 response pointing the client to a new location.
        def WriteResponseRedirect(self, location) :
            headers = { "Location" : location }
            return self.WriteResponse(302, headers, None, None, None)


        # Render the standard HTML error page for a status code.
        ...
        # Send a JSON error response with the supplied object or an empty object.
        ...
        # Send the standard HTTP 400 error page.
        def WriteResponseBadRequest(self) :
            return self.WriteResponseError(400)


        # Send the standard HTTP 403 error page.
        def WriteResponseForbidden(self) :
            return self.WriteResponseError(403)


        # Redirect missing resources when configured, otherwise send HTTP 404.
