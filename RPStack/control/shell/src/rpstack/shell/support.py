...
# Require an attached node before executing runtime-control commands.
...
# Format a byte count as compact B, KB, or MB text.
def human(size):
    if size > 1024 * 1024:
...
# Resolve shell-relative and tilde paths against the device filesystem root/current directory.
def abs_path(path):
    if path.startswith('~'):
        path = '/' + path[1:].lstrip('/')
    if path.startswith('/'):
...
# Normalize directory enumeration across MicroPython and standard Python APIs.
def entries(path):
