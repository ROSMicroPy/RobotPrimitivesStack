...
# Append the signal-specific OTLP path without duplicating an existing suffix.
...
# Normalize a retry delay and sleep when the platform provides a sleep function.
...
# Probe collector reachability with a bounded number of delayed retries.
...
# Set the process-wide switch used by all telemetry signal types.
def _set_enabled(enabled):
    global _TELEMETRY_ENABLED
    _TELEMETRY_ENABLED = bool(enabled)


# Enable telemetry recording and export through the configured providers.
def enable_telemetry():
    _set_enabled(True)
    return True


# Disable recording/export and optionally announce the transition once.
...
# Read the global telemetry enable switch.
def is_telemetry_enabled():
    return bool(_TELEMETRY_ENABLED)


# Select diagnostic or essential tracing and normalize optional span-name prefixes.
...
# Return the currently selected tracing detail mode.
def get_trace_detail_mode():
    return _TRACE_DETAIL_MODE


# Apply the telemetry switch and essential-prefix filter to a span name.
...
# Install the provider used by subsequent tracer lookups.
def set_tracer_provider(provider):
    global _TRACER_PROVIDER
    _TRACER_PROVIDER = provider


# Return the configured provider or an inert provider while telemetry is disabled.
...
# Obtain a named tracer from the currently effective provider.
def get_tracer(name):
    return get_tracer_provider().get_tracer(name)


# Install a logger provider and propagate the existing severity/fallback settings.
...
# Return the configured logger provider or its inert counterpart when disabled.
...
# Obtain a named logger from the currently effective provider.
def get_logger(name):
    return get_logger_provider().get_logger(name)


# Normalize the severity threshold and propagate it to the active logger provider.
...
# Return the configured logging severity threshold.
def get_log_level():
    return _LOG_LEVEL


# Install a fallback sink and propagate it to the logger provider.
...
# Install the provider used by subsequent meter lookups.
def set_meter_provider(provider):
    global _METER_PROVIDER
    _METER_PROVIDER = provider


# Return the configured meter provider or an inert provider when disabled.
...
# Obtain a named meter from the currently effective provider.
def get_meter(name):
    return get_meter_provider().get_meter(name)


# Probe the collector and wire trace, metric, and log providers to synchronous or queued OTLP
# exporters.
def setup_otlp(
    endpoint,
    service_name,
