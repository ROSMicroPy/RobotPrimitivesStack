...
# Configure exporter diagnostic logging and its optional output file.
...
# Read an environment setting through whichever interface the platform exposes.
def _getenv(name):
...
# Combine explicit debug configuration with the OTEL_EXPORT_DEBUG environment flag.
...
# Disable telemetry through the API, falling back to printing the supplied explanation.
...
# Append optional exporter diagnostics without allowing file errors to escape.
...
# Recognize timeout exceptions across host and MicroPython message/errno conventions.
...
# Report non-timeout export failures when debug logging is enabled.
...
# Read an HTTP status through either supported response object convention.
...
# Close an HTTP response without allowing cleanup errors to obscure the result.
...
# Check collector reachability with the available HTTP client and a bounded timeout.
...
# POST an OTLP JSON payload through the available HTTP client and handle export failures.
...
# Encode Python scalar and nested values in OTLP's typed value representation.
...
# Build one OTLP attribute entry from a key and typed value.
def _key_value_item(key, value):
    return {'key': str(key), 'value': _as_otel_value(value)}


# Convert an attribute mapping into OTLP key/value entries.
def _attributes_kv(attributes):
    return [_key_value_item(key, value) for key, value in (attributes or {}).items()]


# Wrap service resource attributes in their OTLP resource representation.
def _resource_data(resource):
    return {'attributes': _attributes_kv(resource)}


# Group exported records under their instrumentation scope, creating the group on demand.
...
# Serialize spans and events into resource- and scope-grouped OTLP trace data.
...
# Serialize log records and trace correlations into an OTLP logs request.
...
# Encode a numeric metric point with timestamps and the appropriate integer/double field.
...
# Convert supported metric aggregates into resource- and scope-grouped OTLP data.
...
class _BaseHTTPExporter:
    # Retain endpoint, request headers, and timeout shared by HTTP exporters.
    def __init__(self, endpoint, headers=None, timeout_s=2):
        self.endpoint = endpoint
        self.headers = headers or {'Content-Type': 'application/json'}
        self.timeout_s = timeout_s

    # Provide the exporter lifecycle hook; this implementation holds no persistent connection.
    def shutdown(self):
        return


class HTTPSpanExporter(_BaseHTTPExporter):
    # Convert spans into OTLP trace JSON and submit it to the configured collector.
...
class HTTPLogExporter(_BaseHTTPExporter):
    # Convert log records into OTLP JSON and submit them to the collector.
...
class HTTPMetricExporter(_BaseHTTPExporter):
    # Convert metric aggregates into OTLP JSON and submit them to the collector.
