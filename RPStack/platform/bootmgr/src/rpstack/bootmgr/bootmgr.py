...
# Recognize ordered S-number_name.py startup scripts.
...
# Extract a parent directory without depending on os.path.
def _dirname(path):
    if not path:
        return ""
    slash = path.rfind("/")
...
# Join directory and filename while handling root and empty prefixes.
...
# Find the first accessible init.d directory among source-relative and device paths.
...
# Execute a startup script in a dedicated namespace and return its exported hooks.
...
# Run recognized startup scripts in sorted order, invoking each exported run hook.
def run():
    init_dir = _resolve_init_dir()
